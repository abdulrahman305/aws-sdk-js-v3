...
} from "../../src/server/operations/DocumentTypeAsMapValue";
import {
 ...
 * Serializes a map that uses documents as the value.
 */
it("DocumentTypeAsMapValueOutput:ServerResponse", async () => {
  class TestService implements Partial<RestJsonService<{}>> {
    DocumentTypeAsMapValue(input: any, ctx: {}): Promise<DocumentTypeAsMapValueServerOutput> {
      const response = {
        docValuedMap: {
          foo: {
            f: 1,
  ...
  const service: any = new TestService();
  const testMux = new httpbinding.HttpBindingMux<"RestJson", keyof RestJsonService<{}>>([
    new httpbinding.UriSpec<"RestJson", "DocumentTypeAsMapValue">("POST", [], [], {
      service: "RestJson",
      operation: "DocumentTypeAsMapValue",
    }),
  ]);
  class TestSerializer extends DocumentTypeAsMapValueSerializer {
    deserialize = (output: any, context: any): Promise<any> => {
      return Promise.resolve({});
    };
  }
